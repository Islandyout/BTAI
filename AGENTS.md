...
changes — it exists so any AI coding agent (Codex, Claude Code, etc.) picking
this up mid-project follows the same conventions as whatever came before.
...
`engine.getModule('name')`. `src/scene/` owns the backend-agnostic entity/component model and JSON scene serialization.
`src/authoring/` owns the validated command surface that tools, editor code, and AI use to
create/inspect/mutate the same scene data.
