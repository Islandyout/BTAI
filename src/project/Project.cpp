...
const std::array<const char*,10> kFolders{"Assets","Scenes","Worlds","Chunks","Prefabs","Materials","Shaders","Scripts","Audio","Animations"};
...
Project Project::create(const std::filesystem::path& root, std::string name) {
  ...
  std::filesystem::create_directories(projectRoot/"Cache",ec);
...
Project Project::open(const std::filesystem::path& manifest) {
  ...
  const auto startup=json.value("startupScene",std::string{"Scenes/Main.btai"});
  return Project(ProjectConfig{name,root,startup});
}
