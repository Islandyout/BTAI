...
void Engine::simulationLoop(std::stop_token token) {
  ...
  registry_.add<ecs::Transform>(entity,ecs::Vec3{0.0f,2.0f,0.0f});
  ...
  streamer_.update({0.0f,0.0f,0.0f});
...
int Engine::run() {
  ...
  while(running_ && !window_->shouldClose()) { window_->poll(); renderer_->draw(); }
  shutdown();
  return 0;
}
...
void Engine::shutdown() noexcept {
  running_=false;
  if(simulation_.joinable()) { simulation_.request_stop(); simulation_.join(); }
  if(renderer_) renderer_->shutdown();
  renderer_.reset(); window_.reset(); jobs_.stop();
}
